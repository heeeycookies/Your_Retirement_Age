...
export function IntroPage({ onStart, currency }: IntroPageProps) {
  return (
    <div className="flex flex-col overflow-hidden" style={{ background: '#FFF8F0' }}>
      ...
      <div
        ...
        style={{ height: 72, background: '#F0D9C4' }}
      >
        <div className="absolute bottom-4 left-0" style={{ animation: 'pixelWalk 16s linear infinite' }}>
          <PixelOtter scale={1.4} walkDuration={99999} />
        </div>
        <div className="absolute bottom-0 left-0 right-0 h-4 border-t-4 border-[#3D2008]" style={{ background: '#C68B57' }} />
      </div>
    </div>
  )
}
